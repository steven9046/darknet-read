...
void *xmalloc_location(const size_t size, const char * const filename, const char * const funcname, const int line) {
    // malloc函数返回的就是void *,空指针，需要对其进行强制类型转换，才能知道里边存的什么数据
...
void *xcalloc_location(const size_t nmemb, const size_t size, const char * const filename, const char * const funcname, const int line) {
    ...
    void *ptr=calloc(nmemb, size);// calloc会给分配内存置为0
    if(!ptr) {
...
// 这里可以把 \t tab \n 换行 \r 空格 0x0d 回车 0x0a 换行 这样的字符都删除，offset的意思就是遇到这样的会往左窜一位
// 例如，  abc\tefg  -->  abcefg
...
char *fgetl(FILE *fp)
{
    if(feof(fp)) return 0;
    size_t size = 512;
    char* line = (char*)xmalloc(size * sizeof(char));// 分配512个char这么大的内存
    if(!fgets(line, size, fp)){ // 读取512个char到line里，读到换行时会停止，所以就是读取一行，这行最多是512个char
        free(line);
        return 0;
    }
