...
{
    // sections这个list就是一个网络了，每一个node是一层，node也是section
    list *sections = read_cfg(filename);
    node *n = sections->front;
    if(!n) error("Config file has no sections", DARKNET_LOC);
    network net = make_network(sections->size - 1);// 创建了一个空的网络
...
list *read_cfg(char *filename)
{
    ...
    section *current = 0;
    while((line=fgetl(file)) != 0){ // 读取一行
        ++ nu;
        strip(line);// 这个应该是删除换行符号等等
        switch(line[0]){
            case '[':
                current = (section*)xmalloc(sizeof(section)); // 创建一个section，把它插入到list里
                list_insert(sections, current);
                current->options = make_list();//这个section里有一个list，装的是Option
                current->type = line; // [convolutional] [shortcut]
                break;
            case '\0':
            case '#':
            case ';': // 一行读完了
